...
for variant in arm64 x64 universal; do
    test -s "$output_dir/MactoyThemer-mac-$variant.dmg"
done
...
Download the DMG for your Mac, open it, and drag MactoyThemer.app to Applications.
...
for variant in ("arm64", "x64", "universal"):
    archive = root / f"MactoyThemer-mac-{variant}.dmg"
